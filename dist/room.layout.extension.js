...
export class Extension {
    ...
    check(roomObject, x, y) {
        const yMax = y + this.topRange;
        const yMin = y - this.botRange;
        const xMax = x + this.rightRange;
        const xMin = x - this.leftRange;
        const structureResults = roomObject.lookForAtArea(LOOK_STRUCTURES, y + this.topRange, x - this.leftRange, y - this.botRange, x + this.rightRange, true);
        if (structureResults.length) {
            return false;
        }
        const terrainResults = _.filter(roomObject.lookForAtArea(LOOK_TERRAIN, yMax, xMin, yMin, xMax, true), (result) => result.terrain != 'plain');
